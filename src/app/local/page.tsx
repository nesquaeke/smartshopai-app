...
export default function LocalDealsPage() {
  ...
  const localDeals = deals.filter((item) => item.dealType === "local");
  ...
  return (
    <main className="relative min-h-screen px-2 pb-10 pt-4 sm:px-4">
      ...
      <div className="mx-auto w-[min(1200px,calc(100%-0.5rem))] space-y-4">
        <Navbar />
        <h1 className="text-2xl font-semibold text-white">{t(locale, "localDeals")}</h1>
        <Card className="p-4">
          <p className="mb-3 text-sm text-white/75">{t(locale, "localDealsHint")}</p>
